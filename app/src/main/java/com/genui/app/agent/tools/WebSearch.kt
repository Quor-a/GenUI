...
object WebSearch {
    ...
    /** Bing：中文结果质量好。显式走 cn.bing.com，避免国际版重定向后结构变化导致抽取失配。 */
    private class Bing(private val http: OkHttpClient) : Engine {
        ...
        override fun search(query: String, want: Int): List<Hit> {
            ...
            val req = Request.Builder()
                .url("https://cn.bing.com/search?q=$q&setlang=zh-hans&count=$want")
                .header("User-Agent", UA)
                .header("Accept-Language", "zh-CN,zh;q=0.9,en;q=0.8")
                .build()
            http.newCall(req).execute().use { resp ->
                ...
                // 主正则落空时退化到通用抽取（百度/必应改版常让 class 匹配失效）
                if (out.isEmpty()) out += genericHits(html)
    ...
    /** 百度：国内可达性最好，作为首选引擎。结果块真实地址在 mu 属性里。 */
    private class Baidu(private val http: OkHttpClient) : Engine {
        override val id = "baidu"
        override val label = "百度"

        override fun search(query: String, want: Int): List<Hit> {
            val q = URLEncoder.encode(query, "UTF-8")
            val req = Request.Builder()
                .url("https://www.baidu.com/s?wd=$q&rn=${want.coerceIn(5, 10)}")
                .header("User-Agent", UA)
            ...
            http.newCall(req).execute().use { resp ->
                ...
                check(html.isNotBlank()) { "响应为空" }
                val out = ArrayList<Hit>()
                for (blk in BD_RESULT_RE.findAll(html)) {
                    if (out.size >= want) break
                    val v = blk.value
                    // 优先用 mu 里的真实地址；否则退而用 href（可能是百度跳转链接）
                    val url = BD_MU_RE.find(v)?.groupValues?.get(1)
                        ?: BD_HREF_RE.find(v)?.groupValues?.get(1)?.let { if (it.startsWith("http")) it else null }
                        ?: continue
                    val title = Text.extract(BD_TITLE_RE.find(v)?.groupValues?.get(1) ?: continue)
                    if (title.isBlank()) continue
                    val snippet = Text.extract(BD_SNIP_RE.find(v)?.groupValues?.get(1) ?: "")
                    out += Hit(title, url, snippet.take(240), id)
                }
                if (out.isEmpty()) out += genericHits(html)
                check(out.isNotEmpty()) { "页面无结果（可能被限流或改版）" }
    ...
    fun search(http: OkHttpClient, query: String, want: Int = 8): JSONObject {
        ...
        val engines = listOf(Baidu(http), Bing(http), Sogou(http), DuckDuckGo(http))
    ...
    /**
     * 通用兜底抽取：主正则落空时，扫描页面里"带可见文本的 http 链接"当作结果。
     * 用于百度/必应改版导致 class 匹配失效的场景。会跳过搜索站自家的导航/跳转链接。
     */
    private fun genericHits(html: String): List<Hit> {
        val out = ArrayList<Hit>()
        for (m in GEN_LINK_RE.findAll(html)) {
            if (out.size >= 8) break
            val url = m.groupValues[1]
            val text = Text.extract(m.groupValues[2])
            if (text.length < 8) continue
            if (url.contains("duckduckgo.com") || url.contains("bing.com/search") ||
                url.contains("baidu.com/s") || url.contains("sogou.com/link")) continue
            out += Hit(text, url, "", "generic")
        }
        return out
    }
    ...
    // ---------------- 百度 ----------------
    private val BD_RESULT_RE = Regex(
        """(?s)<div class="(?:result|c-container)[^"]*"[^>]*>.*?(?=<div class="(?:result|c-container)[^"]*"|<div class="page"|</body>)"""
    )
    private val BD_MU_RE = Regex("""\bmu="(https?://[^"]+)"""")
    private val BD_HREF_RE = Regex("""<a[^>]*href="(https?://[^"]+)"""")
    private val BD_TITLE_RE = Regex("""(?s)<h3[^>]*>.*?<a[^>]*>(.*?)</a>""")
    private val BD_SNIP_RE = Regex("""(?s)<div class="(?:c-abstract|content-right[^"]*|c-row c-span[0-9]+)[^>]*>(.*?)</div>""")

    // ---------------- 通用兜底 ----------------
    private val GEN_LINK_RE = Regex("""(?s)<a[^>]+href="(https?://[^"]+)"[^>]*>(.*?)</a>""")
}
