...
class LLMClient {
 ...
 /** 推理模型思考流：OpenAI 兼容 reasoning_content / Anthropic thinking_delta */
    private fun parseReasoningChunk(payload: String, protocol: Protocol): String? = runCatching {
        when (protocol) {
            Protocol.OPENAI, Protocol.GEMINI -> JSONObject(payload)
                .optJSONArray("choices")?.optJSONObject(0)?.optJSONObject("delta")
                ?.optString("reasoning_content")?.takeIf { it.isNotEmpty() }
            Protocol.ANTHROPIC -> JSONObject(payload).optString("type").let { t ->
                if (t == "content_block_delta") JSONObject(payload)
                    .optJSONObject("delta")?.optString("thinking")?.takeIf { it.isNotEmpty() } else null
            }
        }
    }.getOrNull()
